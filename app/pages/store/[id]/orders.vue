    ...
    <div class="mx-auto flex max-w-7xl flex-col gap-6">
      <el-card class="!rounded-2xl border-0 shadow-sm">
        <div class="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div class="max-w-3xl">
            ...
            <p class="mt-4 text-base leading-7 text-slate-600">Track customer orders, status, and notes for this store.</p>
          </div>
        </div>
      </el-card>
      ...
      <el-card class="!rounded-2xl border-0 shadow-sm" v-loading="orderStore.loading">
        <div class="mb-5 flex justify-between items-center">
          <div><h2 class="text-xl font-semibold">Order Directory</h2><p class="mt-1 text-sm text-slate-500">List of orders retrieved from database.</p></div>
          <el-button round size="small" @click="orderStore.fetchOrders()">Refresh</el-button>
        </div>
        ...
        <el-table :data="orderStore.orders" stripe class="w-full">
          ...
          <el-table-column label="Store" min-width="190"><template #default="{ row }">{{ formatStoreName(row) }}</template></el-table-column>
          <el-table-column label="Customer" min-width="160"><template #default="{ row }">{{ formatCustomerName(row) }}</template></el-table-column>
          <el-table-column label="Currency" min-width="100"><template #default="{ row }">{{ formatCurrencyCode(row) }}</template></el-table-column>
          <el-table-column label="Status" min-width="140"><template #default="{ row }"><el-tag :type="statusType(row.status)" round>{{ row.status }}</el-tag></template></el-table-column>
