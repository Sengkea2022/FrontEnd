  ...
  <div class="space-y-6">
    ...
    <div v-else>
      ...
      <div v-if="showBack" class="mb-4">
        <el-button plain round size="small" @click="$router.push(`/shop/${currentShopUuid}/settings`)">
          ← {{ t('settings') }}
        </el-button>
      </div>
      ...
      <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div class="flex flex-wrap gap-2.5">
          <el-button v-if="isShopOwner" plain round size="default" @click="$router.push(`/shop/${currentShopUuid}/roles`)">
            {{ t('rolesPermissions') }}
          </el-button>
        </div>
      </div>
      ...
      <el-tabs v-model="activeTab" class="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm border border-slate-100 dark:border-slate-700">
        <el-tab-pane label="Current Staff" name="staff">
          <!-- Filter Controls & Invite Staff Action -->
          <div class="mb-4 flex items-center justify-between flex-wrap gap-4">
            ...
            <div class="flex items-center gap-4">
              <span class="text-xs text-slate-400">Total Staff: {{ staffList.length }}</span>
...
const currentShopUuid = computed(() => props.shopUuid || (route.params.id as string) || '')
...
const fetchStoreDepartments = async () => {
  try {
    const res = await fetch<{ data: string[] }>(`/api/user/store-departments?store_uuid=${currentShopUuid.value}`)
...
const isStoreOwner = isShopOwner
const hasStoreAccess = computed(() => {
  ...
  if (authUser.value.shop_code && authUser.value.shop_code !== 'N/A') return true
  return false
})
const userDepartmentScope = computed(() => {
  if (isShopOwner.value) return null
...
const departmentOptions = computed(() => {
  const defaults = ['General', 'Sale', 'Claim', 'Inventory', 'Finance']
  const merged = Array.from(new Set([...defaults, ...dbStoreDepartments.value]))
  return merged
})

const getDepartmentBadgeClass = (dept?: string) => {
  if (!dept) return 'bg-slate-100 text-slate-800 dark:bg-slate-700 dark:text-slate-200'
...
const fetchStaff = async () => {
  ...
  try {
    let url = `/api/user/store-staff?store_uuid=${currentShopUuid.value}`
    ...
    ElNotification({ title: 'Error', message: error.data?.message || 'Failed to fetch staff list', type: 'error' })
  } finally {
...
const fetchRequests = async () => {
  ...
  try {
    const res = await fetch<{ data: any[] }>(`/api/shops/shop-requests?store_uuid=${currentShopUuid.value}`)
    requestsList.value = res.data?.filter((r: any) => r.status === 'pending') || []
  } catch (error: any) {
    ElNotification({ title: 'Error', message: error.data?.message || 'Failed to fetch requests', type: 'error' })
  } finally {
...
const fetchRoles = async () => {
  try {
    const res = await fetch<{ data: any[] }>(`/api/role-permission/roles?paginate=false&store_uuid=${currentShopUuid.value}`)
...
const sendInvite = async () => {
  if (!inviteEmail.value || !inviteRoleId.value) return
  sendingInvite.value = true
  try {
    const targetDept = userDepartmentScope.value || inviteDepartment.value || null
    await fetch('/api/shops/invite-staff', {
      method: 'POST',
      body: {
        email: inviteEmail.value,
        role_id: inviteRoleId.value,
        store_uuid: currentShopUuid.value,
        department: targetDept
      }
    })
    ElNotification({ title: 'Success', message: 'Staff invite sent successfully!', type: 'success' })
    ...
    ElNotification({ title: 'Error', message: error.data?.message || 'Failed to send invite', type: 'error' })
  } finally {
    sendingInvite.value = false
  }
}

const openEditStaffDialog = (staff: any) => {
  editingStaff.value = staff
  editRoleId.value = staff.role_id || staff.role?.id || null
  editDepartment.value = staff.department || ''
  editActiveStatus.value = (staff.active_status === 'active' || staff.active_status === 1 || staff.active_status === '1') ? 'active' : 'inactive'
...
const saveStaffEdit = async () => {
  ...
  try {
    const targetDept = userDepartmentScope.value || editDepartment.value || null
    await fetch(`/api/user/users/${editingStaff.value.id}`, {
      method: 'PUT',
      body: {
    ...
    ElNotification({ title: 'Success', message: 'Staff profile updated successfully!', type: 'success' })
    editStaffDialogVisible.value = false
    fetchStaff()
    fetchStoreDepartments()
  } catch (error: any) {
    ElNotification({ title: 'Error', message: error.data?.message || 'Failed to update staff member', type: 'error' })
  } finally {
    savingStaffEdit.value = false
  }
}

const toggleStaffStatus = async (staff: any, active: boolean) => {
  const newStatus = active ? 'active' : 'inactive'
  try {
    await fetch(`/api/user/users/${staff.id}`, {
      method: 'PUT',
      body: {
        active_status: newStatus
      }
    })
    staff.active_status = newStatus
    ElNotification({ title: 'Status Updated', message: `Staff account ${active ? 'enabled' : 'disabled'}.`, type: 'success' })
  } catch (error: any) {
    ElNotification({ title: 'Error', message: error.data?.message || 'Failed to update staff status', type: 'error' })
    fetchStaff()
  }
}

const kickOut = async (staff: any) => {
  try {
    await ElMessageBox.confirm(
      `Are you sure you want to remove ${staff.name} from this store?`,
      'Warning',
      {
    ...
    ElNotification({ title: 'Success', message: `${staff.name} has been removed from the store.`, type: 'success' })
    fetchStaff()
  } catch (e) {
    // Cancelled
  }
...
const openApproveDialog = (req: any) => {
  ...
  approveDepartment.value = req.department || ''
  approveDialogVisible.value = true
}

const approveRequest = async () => {
  if (!selectedRequest.value || !approveRoleId.value) return
  processingRequest.value = true
  try {
    const targetDept = userDepartmentScope.value || approveDepartment.value || null
    await fetch(`/api/shops/shop-requests/${selectedRequest.value.id}/respond`, {
      method: 'POST',
      body: {
        action: 'accept',
    ...
    ElNotification({ title: 'Success', message: 'Join request approved!', type: 'success' })
    approveDialogVisible.value = false
    fetchRequests()
    fetchStaff()
    fetchStoreDepartments()
  } catch (error: any) {
    ElNotification({ title: 'Error', message: error.data?.message || 'Failed to approve request', type: 'error' })
  } finally {
...
const rejectRequest = async (requestId: number) => {
  try {
    await fetch(`/api/shops/shop-requests/${requestId}/respond`, {
      method: 'POST',
      body: { action: 'reject' }
    })
    ElNotification({ title: 'Success', message: 'Join request rejected.', type: 'info' })
    fetchRequests()
  } catch (error: any) {
    ElNotification({ title: 'Error', message: error.data?.message || 'Failed to reject request', type: 'error' })
  }
}

const cancelInvite = async (inviteId: number) => {
  try {
    await fetch(`/api/shops/shop-requests/${inviteId}/respond`, {
      method: 'POST',
      body: { action: 'reject' }
    })
    ElNotification({ title: 'Success', message: 'Invite cancelled.', type: 'info' })
    fetchRequests()
  } catch (error: any) {
    ElNotification({ title: 'Error', message: error.data?.message || 'Failed to cancel invite', type: 'error' })
  }
}
